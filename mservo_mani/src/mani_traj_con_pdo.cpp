...
#include "mservo_mani/trajectory.h"
...
void commandCallback(const mservo_mani::trajectory::ConstPtr& msg)
{
    ROS_INFO("Callback");
    stringstream ss;
    string data;
    ...
    int pos_desired[g_NbofJoint];
    unsigned int vel_desired[g_NbofJoint];
    unsigned int acc_desired[] = {8000, 8000, 10000, 25000, 10000, 25000, 10000};
    double pos_desired_rad[g_NbofJoint];
    double vel_desired_rad[g_NbofJoint];
//    double acc_desired_rad[traj-1][g_NbofJoint];
    string pos_desired_hex[g_NbofJoint];
    string vel_desired_hex[g_NbofJoint];
    string acc_desired_hex[g_NbofJoint];

    for (int j=0; j<g_NbofJoint; j++){
        // Get and Save Trajectory message
        pos_desired_rad[j] = msg->traj[j];
        vel_desired_rad[j] = msg->traj[j+9];
//        acc_desired_rad[i-1][j] = msg->goal.trajectory.points[i].accelerations[j];
        // Unit Convert
        pos_desired[j] = pos_desired_rad[j]*rad2inc[j];
        if (j == 5){
            if (pos_desired[j]>238000){
                pos_desired[j] = 238000;
            }
        }
        if (j == 1){
            if (pos_desired[j] > 650000){
                pos_desired[j] = 650000;
            }
        }
//        printf("%d, %d\n", j, pos_desired[j]);
//        if (j == 1){
//            vel_desired_rad[j] = vel_desired_rad[j]*0.8;
//        }
//        vel_desired_rad[j] = vel_desired_rad[j]*2.0;
        vel_desired[j] = abs(vel_desired_rad[j]*rads2rpm[j]);

        // Convert HEX to string
        pos_desired_hex[j] = dec_to_hex(pos_desired[j], 4);
        vel_desired_hex[j] = dec_to_hex(vel_desired[j], 4);
        acc_desired_hex[j] = dec_to_hex(acc_desired[j], 4);
    }
    ...
    for (int j = 0; j < 7; j++) {
        ...
        frame.can_dlc = 6;
        data = stringappend(Control_Enable, pos_desired_hex[j]);
        ...
        frame.can_dlc = 8;
        data = stringappend(vel_desired_hex[j], acc_desired_hex[j]);
//        data = vel_desired_hex[j];
        hexstring2data((char *) data.c_str(), frame.data, 8);
        write(s, &frame, sizeof(struct can_frame));
//            ros::Duration(0.00001).sleep();
        sleep(0.00001);

        frame.can_id = COB_ID[j][0];
        frame.can_dlc = 2;
        frame.data[0] = 0x3F;
        frame.data[1] = 0x00;
        write(s, &frame, sizeof(struct can_frame));
...
//    }
    ...
    ss << (finish - begin) << " " ;
//        ss << (finish-begin)<< "  " <<pos_desired_rad[i-1][5] << " " << pos_desired[i-1][5] << " " << pos_desired_hex[i-1][5];
//        ss << " " << vel_desired_rad[i-1][4] << " " << vel_desired[i-1][4]<< " " << vel_desired_hex[i-1][4];
//        ss << " " << acc_desired_rad[i-1][3] << " " << acc_desired[i-1][3]<< " " << acc_desired_hex[i-1][3];
    finish = ros::Time::now();
    ss << (finish-begin);
    LogInfo(ss.str());
...
//    setsockopt(s, SOL_CAN_RAW, CAN_RAW_FILTER, NULL, 0);
    ...
    LogInfo("Set Velocity mode");

    ros::Subscriber sub = n.subscribe("traj", 1000, commandCallback);
    ros::spin();
